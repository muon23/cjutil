...
class NNClassificationModel(ClassifierModel):
    """
    Feed-forward neural network classifier powered by PyTorch.
    """
    ...
    def __init__(
        ...
        if _TORCH_IMPORT_ERROR is not None:
            raise RuntimeError("PyTorch is required for NNClassificationModel") from _TORCH_IMPORT_ERROR
        if not hidden_dims:
    ...
    def fit(self, X: ArrayLike, y: ArrayLike | None = None) -> "NNClassificationModel":
        """
        Train classifier.
        ...
        Returns:
            The same NNClassificationModel instance.
        ...
        classes, y_idx = np.unique(y_arr, return_inverse=True)
        self._classes = classes
        self._model = self._build_network(input_dim=X_arr.shape[1], output_dim=len(classes))
        self._seed_if_needed()
        self._model.train()

        X_tensor = torch.from_numpy(X_arr).to(self.device)  # type: ignore[union-attr]
        y_tensor = torch.from_numpy(y_idx.astype(np.int64)).to(self.device)  # type: ignore[union-attr]
    ...
    def predict_proba(self, X: ArrayLike) -> ArrayLike:
        ...
        with torch.no_grad():  # type: ignore[union-attr]
            logits = model(torch.from_numpy(X_arr).to(self.device))  # type: ignore[union-attr]
            proba = torch.softmax(logits, dim=1)  # type: ignore[union-attr]
        return proba.detach().cpu().numpy()  # type: ignore[union-attr]
    ...
    def set_params(self, **params: Any) -> "NNClassificationModel":
        """
